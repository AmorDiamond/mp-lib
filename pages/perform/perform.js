...
Page({
  handelsreach(e){
   ...
   showList(statusC="未确认",code=""){
    ...
    let brandCodes = companyList.map(item => item.brandCode)
     const data = {
       ...
       size:10,
       brandCodes,
       budgCompC:wx.getStorageSync('agentCode')
     }
   ...
   handeltoDetail(e){
    //  console.log(e)
     ...
     let budgCompC = e.currentTarget.dataset.namec
     wx.navigateTo({
       url: `/pages/performDetail/performDetail?name=${costTypeC}&code=${code}&content=${content}&id=${id}&checkTime=${checkTime}&budgCompC=${budgCompC}`
     })
   ...
   hanonClosedelok(e){
    this.setData({showSide:false,page:1,totalPages:1,List:[],inputValue:""})
    if(this.data.showBorder){
      this.showList("未确认",this.data.inputValue)
    }else if(this.data.showBorder==false){
      this.showList("已确认",this.data.inputValue)
  ...
  onLoad: function (options) {
     this.showList()
     let brandlist = wx.getStorageSync('companyList')
     this.setData({
      searchList:brandlist,
      brandCode:brandlist[0].brandCode
     })
  },
