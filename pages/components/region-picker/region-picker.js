...
Component({
    properties: {
        ...
        regionValue: {
            ...
            observer: function (newVal, oldVal) {
                if (newVal.length > 0) {
                    ...
                    this.setData({
                        ['region.tabs']: newVal,
                        ['region.select']: select < number ? select + 1 : select,
                    }, () => {
                        this.setData({
                            area: this.getChildArea(select < number ? select + 1 : select),
                        });
                    });
                }
            },
        ...
        area: {
            ...
            value: [],
            observer: function (newVal, oldVal) {
                if (!this.data.oldArea.length) {
                    this.setData({oldArea: newVal});
                    if (this.data.regionValue.length) {
                        this.setData({area: this.getChildArea(this.data.number)});
                    }
                }
            },
        },
        number: {
            ...
            observer: function (newVal, oldVal) {
                if (newVal !== oldVal) {
                    this.data.region.tabs.filter((item, i) => i <= newVal)
    ...
    methods: {
        ...
        emitHideRegion: function () {
            let number = this.data.number;
            if (this.data.region.tabs[number].id === '') {
                wx.showToast({
                    title: '请选择完整省市区',
                    icon: 'none',
                    duration: 2000,
        ...
        bindRegionChange: function (e) {
            ...
            // 除了三级以外的需要获取对应子选项
            let number = this.data.number;
            if (this.data.region.select < number) {
                this.setData({
